...
class CGAN_changed1():
    def __init__(self):
        ...
        optimizer_g = Adam(0.0002, 0.5) # we will set the second optimizer for generator for jusitification
    ...
    def build_discriminator(self):
        ...
        label_embedding = Flatten()(Embedding(self.num_classes, np.prod(self.img_shape))(label))    # 展平标签
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        img_dir = r"/content/images_change1"
        ...
        for epoch in range(epochs):     
            ...
            imgs, labels = X_train[idx], y_train[idx]   # imgs.shape --> (batch_size,255,255,1)
    ...
    def sample_images(self, epoch, epochs, img_dir):
        if epoch == epochs-1:
            # save 50 images
            final_img_dir = os.path.join(self.gdrive_path, 'MyDrive', 'cgan_Assessment', 'Generated_covid_imgs_change1')
            ...
            for i in range(r):
                for j in range(c):
                    ...
                    axs[i][j].axis('off')
                    cnt+= 1
            fig.savefig("images_change1/%d.png" % epoch)
            plt.close()
